...
func _process(delta):
	if is_network_master():
		...
		$Base/Turret.look_at(get_global_mouse_position())
		$Base/Turret.rotate(PI/2)
		...
		if Input.is_action_pressed("Fire_small") and $ProgressBar.value >= 100 and bulls<maxBulls:
			var bname = str(randi())
