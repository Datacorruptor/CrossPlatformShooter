...
func _player_disconnected(id):
	if has_node("/root/gamescreen"): # Game is in progress.
...
remote func pre_start_game():
	# Change scene.
	var world = load("res://gamescreen.tscn").instance()
...
func end_game():
	for i in get_tree().get_root().get_children():
		if i.is_in_group("bullet") or i.is_in_group("powerup"):
			i.queue_free()
	if has_node("/root/gamescreen"): # Game is in progress.
		# End it
...
remote func kicked(reason):
	emit_signal("game_error", "You have been banned!!!")
